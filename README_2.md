...
| Google Books da **429** | Falta `GOOGLE_BOOKS_KEY` |
| Google Books da **403** | Ver abajo: casi siempre la API sin habilitar o la clave restringida por referente |
| Open Library: «no responde desde el servidor de la app» | Open Library bloquea las IP de centros de datos. No es culpa tuya y no tiene arreglo desde aquí |
| Una columna concreta falla en todas las filas | Su tipo en Notion no admite ese valor. El aviso dice cuál es |

### El 403 de Google Books

Un 403 **con clave puesta** significa que la clave llega pero Google la rechaza.
Dos causas, por orden de frecuencia:

1. **La Books API no está habilitada** en el proyecto de la clave.
   Consola de Google Cloud → *APIs y servicios* → *Biblioteca* → busca **Books API**
   → **Habilitar**. Comprueba arriba que el proyecto seleccionado es el mismo
   donde creaste la clave.
2. **La clave tiene restricción de aplicación.** Si en *Credenciales* → tu clave →
   *Restricciones de aplicación* está marcado **Sitios web (referentes HTTP)**,
   las llamadas desde un servidor fallan siempre: no hay navegador ni referente.
   Ponlo en **Ninguna**. La restricción de *API* (limitarla a Books API) sí
   conviene dejarla: esa protege sin romper nada.

El mensaje exacto de Google aparece ahora en la app, bajo **Fuentes que han fallado**.
